    ...
    <AppShell>
      <div className="grid items-center gap-6 lg:grid-cols-[1fr_.9fr]">
        <div>
          ...
          <div className="mt-7 flex flex-wrap gap-4"><button onClick={() => setModal({ title: "List Your Project", description: "This opens an Investor Preview listing flow for project campaigns. No payment or contract interaction is performed." })} className="purple-button rounded-xl px-6 py-3">List Your Project</button><button onClick={() => setModal({ title: "How It Works", description: "Projects launch Simulated Market Data markets, activate creators, and track simulated community growth." })} className="rounded-xl border border-white/10 bg-white/5 px-6 py-3">How It Works</button></div>
        </div>
      ...
      <div className="mt-5 grid grid-cols-2 gap-2 rounded-2xl border border-white/10 bg-white/5 p-2 md:grid-cols-5 lg:grid-cols-10">
        {categories.map((cat, i) => <button key={cat} onClick={() => setModal({ title: cat, description: "Project category filters are simulated for the Investor Preview." })} className={`interactive-glow rounded-xl py-3 text-sm ${i === 0 ? "bg-neon" : "hover:bg-white/5"}`}>{cat}</button>)}
      </div>

      <GlassCard className="mt-5 p-5">
        <div className="mb-4 flex justify-between"><h2 className="text-xl font-semibold">Top Gaining Projects</h2><button onClick={() => setModal({ title: "Top Gaining Projects", description: "This would open a full project leaderboard with Live Preview growth movement." })} className="text-purple-300">View All</button></div>
      ...
      <section className="mt-5 grid gap-5 lg:grid-cols-[.8fr_1.2fr]">
        <GlassCard className="p-5">
          <div className="mb-4 flex justify-between"><h2 className="text-xl font-semibold">Featured Projects</h2><button onClick={() => setModal({ title: "Filters", description: "Advanced filters are visual-only in this Preview Environment." })} className="rounded-lg border border-white/10 px-3 py-1 text-sm"><Filter className="mr-1 inline h-4 w-4" />Filters</button></div>
      ...
      <PreviewModal open={!!modal} title={modal?.title ?? ""} description={modal?.description} onClose={() => setModal(null)}>
        <div className="rounded-2xl border border-cyan/20 bg-cyan/10 p-4 text-sm text-slate-200">Project growth, campaigns, and rewards are Simulated Market Data only.</div>
      </PreviewModal>
    </AppShell>
  );
}
