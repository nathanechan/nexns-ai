    ...
    <AppShell>
      ...
      <section className="mt-5 grid gap-5 lg:grid-cols-[.9fr_.9fr_1.4fr]">
        <GlassCard className="interactive-glow p-6 md:p-7">
          ...
          <button onClick={() => setModal({ title: "Growth Progress", description: "This opens a Preview Environment level progress panel. EXP, badges, and boosts are simulated." })} className="interactive-glow mt-6 flex w-full items-center justify-center gap-2 rounded-xl bg-neon/15 py-4">View Progress <ArrowRight className="h-4 w-4" /></button>
        </GlassCard>
      ...
      <GlassCard className="mt-5 grid gap-5 p-6 lg:grid-cols-[1fr_260px]">
        ...
        <GlassCard className="bg-neon/15 p-6"><h2 className="text-2xl font-bold">Invite Friends</h2><p className="mt-2 text-slate-300">Earn 20% from their earnings.</p><button onClick={() => setModal({ title: "Invite Friends", description: "A Investor Preview referral panel would open here. No invite is sent from this Preview Environment." })} className="purple-button mt-6 rounded-xl px-6 py-3">Invite Now</button></GlassCard>
      </GlassCard>

      <PreviewModal open={!!modal} title={modal?.title ?? ""} description={modal?.description} onClose={() => setModal(null)}>
        <div className="rounded-2xl border border-cyan/20 bg-cyan/10 p-4 text-sm text-slate-200">Simulated Market Data only. No real payments, wallet actions, or token transfers.</div>
      </PreviewModal>
    </AppShell>
  );
}
